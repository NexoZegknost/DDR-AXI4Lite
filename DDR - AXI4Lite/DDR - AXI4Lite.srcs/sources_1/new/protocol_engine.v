module protocol_engine (
        ...
        case (current_state)
            ...
            STATE_READ, STATE_WRITE: begin
                if (timer_reg == 8'd0) begin
                    ...
                    next_timer = 8'd3; // Trễ tRP (3 chu kỳ đóng hàng)
                end
            end
    ...
    // Output Control Logic: Đồng bộ phát lệnh vật lý theo sự chuyển dịch trạng thái
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Đặt lại các giá trị mặc định (Strobe/Pulse signals)
            cmd_ready       <= 1'b0;
            ref_ack         <= 1'b0;
            read_data_valid <= 1'b0;
            {mem_cs_n, mem_ras_n, mem_cas_n, mem_we_n} <= CMD_NOP;

            case (next_state) // KIỂM TRA TRẠNG THÁI TIẾP THEO ĐỂ ĐẨY LỆNH NGAY CHU KỲ CHUYỂN
                STATE_IDLE: begin
                    if (!ref_req && cmd_valid && (current_state == STATE_IDLE)) 
                        cmd_ready <= 1'b1;
                end
                
                STATE_ACTIVATE: begin
                    // Bắn lệnh ACTIVATE ngay lập tức khi FSM chuẩn bị bước vào trạng thái này
                ...
                STATE_READ: begin
                    if (current_state == STATE_ACTIVATE) begin
                        {mem_cs_n, mem_ras_n, mem_cas_n, mem_we_n} <= CMD_READ;
                        mem_ba   <= req_bank;
                        mem_addr <= req_col;
                    end
                    // Đọc data từ chân DQ về sau một khoảng trễ
    ...
    // Gán mạch ba trạng thái cho bus dữ liệu DQ
    assign mem_dq = (current_state == STATE_WRITE) ? cmd_wdata : 32'hZZZZ_ZZZZ;
